...
void IrrDriver::renderGBuffer()
{
...

    {
        ...
        // Skinned meshes
        {
            ...
            for (STKAnimatedMesh *animatedmesh : ListSkinned::getInstance()->SolidPass)
            {
...
void IrrDriver::renderShadows()
{
    ...
    for (unsigned cascade = 0; cascade < 4; cascade++)
    {
        ...
        // Skinned meshes
        {
            if (CVS->isARBBaseInstanceUsable())
                glBindVertexArray(VAOManager::getInstance()->getVAO(video::EVT_STANDARD, true));
            for (STKAnimatedMesh *animatedmesh : ListSkinned::getInstance()->Shadows[cascade])
            {
                core::matrix4 ModelMatrix = animatedmesh->getAbsoluteTransformation(), InvModelMatrix;
                ModelMatrix.getInverse(InvModelMatrix);

                glUseProgram(MeshShader::SkinnedObjectShadowShader::getInstance()->Program);
                for (GLMesh *mesh : animatedmesh->MeshSolidMaterial[Material::SHADERTYPE_SOLID])
                {
                    if (!CVS->isARBBaseInstanceUsable())
                        glBindVertexArray(mesh->vao);
                    MeshShader::SkinnedObjectShadowShader::getInstance()->setUniforms(cascade, ModelMatrix, *(animatedmesh->JointMatrixes));
                    glDrawElementsBaseVertex(mesh->PrimitiveType, (int)mesh->IndexCount, mesh->IndexType, (GLvoid *)mesh->vaoOffset, (int)mesh->vaoBaseVertex);
                }

                glUseProgram(MeshShader::SkinnedRefObjectShadowShader::getInstance()->Program);
                for (GLMesh *mesh : animatedmesh->MeshSolidMaterial[Material::SHADERTYPE_ALPHA_TEST])
                {
                    if (!CVS->isARBBaseInstanceUsable())
                        glBindVertexArray(mesh->vao);
                    if (!CVS->isAZDOEnabled())
                        MeshShader::SkinnedRefObjectShadowShader::getInstance()->SetTextureUnits(getTextureGLuint(mesh->textures[0]));
                    else
                        MeshShader::SkinnedRefObjectShadowShader::getInstance()->SetTextureHandles(mesh->TextureHandles[0]);
                    MeshShader::SkinnedRefObjectShadowShader::getInstance()->setUniforms(cascade, ModelMatrix, *(animatedmesh->JointMatrixes));
                    glDrawElementsBaseVertex(mesh->PrimitiveType, (int)mesh->IndexCount, mesh->IndexType, (GLvoid *)mesh->vaoOffset, (int)mesh->vaoBaseVertex);
                }
            }
        }
